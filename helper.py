...
class Helper:
    ...
    def get_trailer_id(self, soup: BeautifulSoup) -> str:
        try:
            ...
            data_src = iframe.get("data-src", iframe.get("src"))
    ...
    def get_cover_url(self, href: str, detail_page_infor: BeautifulSoup) -> str:
        try:
            dm_thumb = detail_page_infor.find("div", class_="dp-i-c-poster")
    ...
    def get_extra_info(self, detail_page_infor: BeautifulSoup) -> dict:
        extra_info = {}
        try:
            quality = detail_page_infor.find("span", class_="quality")
            extra_info["quality"] = quality.text
        except:
            pass

        try:
            imdb = detail_page_infor.find("span", class_="imdb")
            ratingValue = imdb.find("span", {"itemprop": "ratingValue"})
            print(f"Got ratingValue: {ratingValue.text}")
            extra_info["imdb"] = ratingValue.text

            a = ratingValue.find("a")
            if a:
                href = a.get("href", "")
